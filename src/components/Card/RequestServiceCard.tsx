...
const RequestServiceCard: React.FC<RequestCardProps> = ({ request }) => {
    const getColor = (status: ServiceStatus) => {
        const statusText = status.toLowerCase()
        if (statusText === ServiceStatus.ACCEPTED.toLowerCase()) {
            return <Chip label={statusText} color='success' />
        } else if (statusText === ServiceStatus.REQUESTED.toLowerCase()) {
            return <Chip label={statusText} color='warning' />
        } else if (statusText === ServiceStatus.DECLINED.toLowerCase()) {
            return <Chip label={statusText} color='error' />
        } else if (statusText === ServiceStatus.COMPLETED.toLowerCase()) {
            return <Chip label={statusText} color='primary' variant='outlined' />
        }
